...
- `wrangler` — 运行 `pnpm add -g wrangler`，然后 `wrangler login` 登录一次

### 一键部署（推荐）

运行自动化部署脚本，它会自动完成所有步骤：
...
脚本会自动：
1. 启动后端服务器
2. 开启 Cloudflare Tunnel 并获取隧道 URL
3. 将隧道 URL 写入 `src/hooks/useGameConnection.ts`
4. 编译前端（`pnpm build`）
5. 部署到 Cloudflare Pages
6. 退出时自动恢复源文件

部署完成后，将终端输出的 Cloudflare Pages 链接发给朋友即可。按 `Ctrl+C` 停止服务器和隧道。
